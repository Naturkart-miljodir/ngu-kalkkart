...
PRED_OUT = r"C:\Users\acosta_pedro\OneDrive - Norges geologiske undersøkelse\Geochemistry NGU_2026\Kalk_project\Modelling\Nord_Trondelag\Raster_predictions"
CHANNEL_MAP_CSV = r"E:\Test\National_test\DL_tiles_5pct_noSoftBgd\DL_AE_chips\tile_metadata\channel_map.csv"
...
# ---- categorical input channel names expected in channel_map.csv ----
...
def match_predictor_files(predictor_dir, channel_map_csv, predictor_list_file=None):
    ...
    return ordered_names, ordered_specs
...
def open_predictor_datasets(channel_specs):
    unique_paths = []
    seen = set()
    for spec in channel_specs:
        p = spec["path"]
    ...
    datasets = {p: rasterio.open(p) for p in unique_paths}
...
def read_block_stack(datasets, channel_specs, row0, row1, col0, col1):
    ...
    for i, spec in enumerate(channel_specs):
        ds = datasets[spec["path"]]
        band = int(spec.get("band", 1))
        arr = ds.read(band, window=win, boundless=True, fill_value=np.nan)
...
def split_inputs_for_model(block_stack, quaternary_idx, landuse_idx):
...
def predict_block(
    ...
    for r in rows:
        for c in cols:
            ...
            cont, qua, land = split_inputs_for_model(tile_filled, quaternary_idx, landuse_idx)
...
def write_block_outputs(outputs, row0, col0, prob_block):
    ...
    win = Window(col0, row0, block_w, block_h)
...
def main():
    ...
    ordered_names, channel_specs = match_predictor_files(
        PREDICTOR_DIR,
        CHANNEL_MAP_CSV,
    ...
    print(f"\nRaster stack validated: {len(channel_specs)} channel-map rows | size={width} x {height}")
    ...
    outputs = create_output_rasters(ref_profile, PRED_OUT)
    datasets = open_predictor_datasets(channel_specs)
    ...
    try:
        ...
        for row0_core in row_starts:
            row1_core = min(row0_core + SUPERBLOCK_SIZE, height)
            for col0_core in col_starts:
