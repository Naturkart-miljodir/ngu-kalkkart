...
PERMUTATION_N_REPEATS = 3          # Set to 3 for more stable estimates
...
def build_permutation_plan(
    ...
    # Explicit embedding groups only if embeddings are enabled
...
if DO_SPATIAL_CV:
    ...
    for fold in range(N_FOLDS):
        ...
        # =========================
        # PERMUTATION TEST PER FOLD
        # =========================
        if DO_PERMUTATION_TEST and len(permutation_plan) > 0:
            print(f"\n=== Running permutation test for Spatial Fold {fold+1} ===")

            fold_perm_dir = os.path.join(permutation_fold_dir, f"fold_{fold+1}")
            os.makedirs(fold_perm_dir, exist_ok=True)

            baseline_metrics, fold_rows = run_permutation_test_for_fold(
                model=best_model,
                fold_idx=fold + 1,
                Xva=Xva_clean,
                yva=yva,
                permutation_plan=permutation_plan,
                permutation_out_dir=fold_perm_dir,
                n_repeats=PERMUTATION_N_REPEATS,
                random_seed=PERMUTATION_RANDOM_SEED,
                Xva_qua=Xva_qua_clean,
                Xva_land=Xva_land_clean,
                batch_size=2,
            )

            all_permutation_rows.extend(fold_rows)
            print(f"[PERM] Fold {fold+1}: wrote {len(fold_rows)} permutation rows")
        elif DO_PERMUTATION_TEST:
            print(f"[PERM] Fold {fold+1}: permutation test skipped because permutation_plan is empty")
