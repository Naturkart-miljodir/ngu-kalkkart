...
from keras.layers import Conv2D, MaxPooling2D, UpSampling2D, Input, Concatenate, Dropout, Embedding, Lambda
...
DERIVED_CHANNEL_IDX = [11, 12, 13, 14, 15]  # NDVI, NDWI, NDMI, NDSI, NBR (now properly normalized)
...
if len(X_paths) == 0 or len(y_paths) == 0 or len(qua_paths) == 0 or len(land_paths) == 0:
...
if not (len(X_paths) == len(y_paths) == len(qua_paths) == len(land_paths)):
    ...
    print(f"  X: {len(X_paths)}, y: {len(y_paths)}, quaternary: {len(qua_paths)}, landuse: {len(land_paths)}")
    exit(1)
...
quaternary = np.array(qua_cleaned, dtype=np.int32)
...
landuse = np.array(land_cleaned, dtype=np.int32)

quaternary = quaternary[..., np.newaxis]
landuse = landuse[..., np.newaxis]
...
def build_unet_with_embeddings(input_shape, num_classes):
    ...
    model_input = Concatenate(axis=-1, name="concat_cont_embeddings")([cont_in, qua_feat, land_feat])
...
class F1MetricsCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        pred_counts = np.array([np.sum(y_pred == cls) for cls in [1, 2, 3]], dtype=np.int64)
        true_counts = np.array([np.sum(y_true == cls) for cls in [1, 2, 3]], dtype=np.int64)
...
def compute_mc_uncertainty(model, X_cont, X_qua, X_land, valid_mask=None, mc_iterations=MC_ITERATIONS, mc_batch_size=MC_BATCH_SIZE):
    """
    ...
    for i in tqdm(range(mc_iterations), desc="MC Dropout passes", unit="pass"):
        ...
        for start_idx in range(0, n_samples, mc_batch_size):
            ...
            X_cont_batch = tf.convert_to_tensor(X_cont[start_idx:end_idx], dtype=tf.float32)
...
def create_dataset(X_cont, X_qua, X_land, y, batch_size=GLOBAL_BATCH_SIZE, shuffle=True, drop_remainder=True):
...
if DO_SPATIAL_CV:
    ...
    for fold, (tr_idx, va_idx) in enumerate(kf.split(X), start=1):
        ...
        print(f"  [DEBUG CV FOLD {fold}] Xva after split - has NaN: {np.isnan(Xva_cont).any()}, range: [{np.nanmin(Xva_cont):.4f}, {np.nanmax(Xva_cont):.4f}]")
        ...
        f1cb = F1MetricsCallback(Xva_cont, Xva_qua, Xva_land, yva, f1_log, prefix=f"CV Fold {fold}")

        # Create tf.data.Dataset for efficient GPU streaming
        train_dataset = create_dataset(Xtr_cont, Xtr_qua, Xtr_land, ytr, batch_size=GLOBAL_BATCH_SIZE, shuffle=True, drop_remainder=True)
        val_dataset = create_dataset(Xva_cont, Xva_qua, Xva_land, yva, batch_size=GLOBAL_BATCH_SIZE, shuffle=False, drop_remainder=True)
        ...
        print(f"  [DEBUG CV FOLD {fold}] After fit - Xva has NaN: {np.isnan(Xva_cont).any()}, Xva_clean has NaN: {np.isnan(Xva_cont_clean).any()}")
...
if DO_MAIN_TRAIN:
    ...
    Xtr_cont, Xva_cont, Xtr_qua, Xva_qua, Xtr_land, Xva_land, ytr, yva = train_test_split(
    ...
    Xva_qua_clean = Xva_qua.copy()
    Xva_land_clean = Xva_land.copy()
    print(f"  [DEBUG MAIN] Xva after split - has NaN: {np.isnan(Xva_cont).any()}, range: [{np.nanmin(Xva_cont):.4f}, {np.nanmax(Xva_cont):.4f}]")
    ...
    f1cb = F1MetricsCallback(Xva_cont, Xva_qua, Xva_land, yva, f1_main_log, prefix="MAIN")
    ...
    if USE_MC_DROPOUT:
        ...
        print(f"  [DEBUG MAIN] Before MC - Xva has NaN: {np.isnan(Xva_cont).any()}, Xva_clean has NaN: {np.isnan(Xva_cont_clean).any()}")
