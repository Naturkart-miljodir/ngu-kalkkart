...
# --- Helper: parse_channel_indices ---
def parse_channel_indices(idx):
    """
    Parse a list of indices or a string with ranges (e.g., '1,2,3,4' or '1:4,6,8:10') into a list of ints.
    :param idx: list of ints or string
    :return: list of ints
    """
    if idx is None:
        return []
    ...
    if isinstance(idx, str):
        ...
        return result
    raise ValueError("Indices must be a list of ints or a string like '1:4,6,8:10'")

# --- Helper: parse_excluded_predictors_and_indices ---
def parse_excluded_predictors_and_indices(channels_excl):
    """
    Accepts CHANNELS_EXCL as False/None, string, int, or list/tuple/set of names/indices/ranges.
    Returns a tuple: (predictor_names, channel_indices)
    """
    if channels_excl is False or channels_excl is None:
        return [], []
    if isinstance(channels_excl, int):
        return [], [channels_excl]
    if isinstance(channels_excl, str):
        # If the string contains only digits, colons, or commas, treat as indices
        if re.fullmatch(r"[\d:,\s]+", channels_excl):
            return [], parse_channel_indices(channels_excl)
        else:
            return [channels_excl.lower()], []
    if isinstance(channels_excl, (list, tuple, set)):
        names = []
        indices = []
        for item in channels_excl:
            if isinstance(item, int):
                indices.append(item)
            elif isinstance(item, str):
                s = item.strip()
                if re.fullmatch(r"[\d:,\s]+", s):
                    indices.extend(parse_channel_indices(s))
                else:
                    names.append(s.lower())
            else:
                raise ValueError("CHANNELS_EXCL entries must be int or str")
        return names, indices
    raise ValueError("CHANNELS_EXCL must be False, int, string, or a list/tuple/set of names/indices")
    ...
    if isinstance(idx, str):
        ...
        for part in idx.split(','):
            ...
            if re.match(r"^\d+:\d+$", part):
                ...
                result.append(int(part))
        return result
    raise ValueError("CHANNELS_INCLUDE_IDX must be a list of ints or a string like '1:4,6,8:10'")
...
N_FOLDS = 3
DO_MAIN_TRAIN = True
FOLD_ASSIGNMENT_MODE = "size_only"  # options: "size_only" if large area used, "class_aware" if small area used

# === PATCHED HYPERPARAMETERS ===
CV_EPOCHS = 25
LEARNING_RATE = 1e-4
CLASS2_WEIGHT_MULTIPLIER = 1.8
MAIN_EARLY_STOP_PATIENCE = 8
MAIN_LR_PATIENCE = 3
MAIN_LR_FACTOR = 0.5
MAIN_MIN_LR = 1e-6
...
# Set to None to auto-detect from the loaded tile data when embeddings are enabled.
AUTO_DETECT_CATEGORICAL_NUM_CLASSES = True
QUATERNARY_NUM_CLASSES = None
LANDUSE_NUM_CLASSES = None
QUATERNARY_EMBED_DIM = 5
LANDUSE_EMBED_DIM = 6
QUATERNARY_PREDICTOR_NAMES = ["QuaternaryClass_id"]
LANDUSE_PREDICTOR_NAMES = ["landuseCode_18_cog"]
REMOVE_EMBED_CHANNELS_FROM_CONTINUOUS = True

ACTIVE_QUATERNARY_NUM_CLASSES = 1
ACTIVE_LANDUSE_NUM_CLASSES = 1

# Channel exclusion list (substring/prefix matching using channel_map.csv predictor names)
CHANNELS_EXCL = False
USE_PREDICTOR_INCLUDE_CSV = True
PREDICTOR_INCLUDE_CSV = r"D:\Unet_models\MidNorge\250K\With_Ca_class\Embeddings\predictor_list_used_for_training.csv"

#CHANNEL NAMES OR INDICES CAN BE USED AS INPUT 
# # ALPHA EARTH CHANNELS TO REMOVE FROM OCCLUSION TEST INDEX = "15,16,19,21:24,26:38,41,42,44:46,48,50,53,55,56,58,60,61,63:75,77,78"
# ALPHA EARTH CHANNELS TO REMOVE FROM CHANNEL MAP INDEX = "17,18,24:26,28:35,37,39,50,55,57,58,63,65,67,68,70,73,77,80
...
if USE_ADVANCED_LOSS and ADVANCED_LOSS_TYPE == "focal_tversky":
    print(f"Focal-Tversky: alpha={FT_ALPHA}, beta={FT_BETA}, gamma={FT_GAMMA}")
else:
    print("Focal-Tversky params: inactive")
...
if USE_CATEGORICAL_EMBEDDINGS:
    print("AUTO_DETECT_CATEGORICAL_NUM_CLASSES:", AUTO_DETECT_CATEGORICAL_NUM_CLASSES)
    print("QUATERNARY_NUM_CLASSES (config):", QUATERNARY_NUM_CLASSES)
    print("LANDUSE_NUM_CLASSES (config):", LANDUSE_NUM_CLASSES)
    print("QUATERNARY_EMBED_DIM:", QUATERNARY_EMBED_DIM)
    print("LANDUSE_EMBED_DIM:", LANDUSE_EMBED_DIM)
    print("QUATERNARY_PREDICTOR_NAMES:", QUATERNARY_PREDICTOR_NAMES)
    print("LANDUSE_PREDICTOR_NAMES:", LANDUSE_PREDICTOR_NAMES)
    print("REMOVE_EMBED_CHANNELS_FROM_CONTINUOUS:", REMOVE_EMBED_CHANNELS_FROM_CONTINUOUS)
else:
    print("Embedding config: disabled")
print("CHANNELS_EXCL:", CHANNELS_EXCL)
print("USE_PREDICTOR_INCLUDE_CSV:", USE_PREDICTOR_INCLUDE_CSV)
print("PREDICTOR_INCLUDE_CSV:", PREDICTOR_INCLUDE_CSV)
...
print("MAIN_EARLY_STOP_PATIENCE:", MAIN_EARLY_STOP_PATIENCE)
print("MAIN_LR_PATIENCE:", MAIN_LR_PATIENCE)
print("MAIN_LR_FACTOR:", MAIN_LR_FACTOR)
print("MAIN_MIN_LR:", MAIN_MIN_LR)
print("SPATIAL_BLOCK_SIZE_KM:", SPATIAL_BLOCK_SIZE_KM)
print("SPATIAL_BUFFER_BLOCKS:", SPATIAL_BUFFER_BLOCKS)
print("FOLD_ASSIGNMENT_MODE:", FOLD_ASSIGNMENT_MODE)
if FOLD_ASSIGNMENT_MODE == "class_aware":
    print("CLASS_AWARE_FOLD_WEIGHTS:", CLASS_AWARE_FOLD_WEIGHTS)
print()
...
TILE_DIR = r"D:\DL_AE_chips\Clean_data_ed_2"
OUT_ROOT = r"D:\Unet_models\MidNorge\250K\With_Ca_class\Embeddings"
MODEL_OUT = os.path.join(OUT_ROOT, "models")
QC_PLOTS = os.path.join(OUT_ROOT, "qc_plots")
PREDICTOR_LIST_USED_CSV = os.path.join(OUT_ROOT, "predictor_list_used_for_training.csv")

SPATIAL_METADATA_DIR_CANDIDATES = [
    r"D:\DL_AE_chips\Clean_data_ed_2\tile_metadata",
    r"D:\DL_AE_chips\tile_metadata",
    os.path.join(TILE_DIR, "tile_metadata"),
...
def export_predictor_list_used(csv_path, channel_map_rows, continuous_orig_indices, embedding_rows=None):
    if not channel_map_rows:
        print("[WARNING] Channel map unavailable. Skipping predictor list export.")
        return

    fieldnames = list(channel_map_rows[0].keys()) + ["input_branch", "original_channel_idx", "final_channel_idx"]
    export_rows = []

    for final_idx, orig_idx in enumerate(continuous_orig_indices):
        row = dict(channel_map_rows[orig_idx])
        row["input_branch"] = "continuous"
        row["original_channel_idx"] = orig_idx
        row["final_channel_idx"] = final_idx
        export_rows.append(row)

    for embedding in embedding_rows or []:
        row = dict(channel_map_rows[embedding["orig_idx"]])
        row["input_branch"] = embedding["branch"]
        row["original_channel_idx"] = embedding["orig_idx"]
        row["final_channel_idx"] = ""
        export_rows.append(row)
    ...
    print(f"[INFO] Saved predictor list used for training: {csv_path}")
...
def resolve_first_available_channel_idx(by_predictor, predictor_names, label):
    for predictor_name in predictor_names:
        key = predictor_name.strip().lower()
        idxs = by_predictor.get(key, [])
        if len(idxs) == 1:
            return idxs[0], predictor_name
        if len(idxs) > 1:
            raise RuntimeError(
                f"{label} predictor '{predictor_name}' has multiple channels {idxs}; expected a single categorical channel"
            )
    raise RuntimeError(f"None of the configured {label} predictors were found in channel map: {predictor_names}")


def resolve_categorical_num_classes(arr, configured_num_classes, label):
    configured = int(configured_num_classes) if configured_num_classes is not None else None
    if configured is not None and not AUTO_DETECT_CATEGORICAL_NUM_CLASSES:
        return max(1, configured)

    arr_int = np.rint(np.nan_to_num(arr, nan=0.0)).astype(np.int32)
    arr_int = np.where(arr_int < 0, 0, arr_int)
    detected = int(arr_int.max()) if arr_int.size else 0
    resolved = max(1, detected)
    if configured is not None:
        resolved = max(resolved, configured)
    print(f"[INFO] {label} class count resolved: {resolved} (configured={configured}, detected={detected})")
    return resolved
...
def load_included_channel_indices_from_csv(include_csv_path, channel_map_by_predictor):
    if not include_csv_path or not os.path.exists(include_csv_path):
        raise FileNotFoundError(f"Predictor include CSV not found: {include_csv_path}")

    with open(include_csv_path, "r", encoding="utf-8") as f:
        rows = list(csv.DictReader(f))

    if not rows:
        raise RuntimeError(f"Predictor include CSV is empty: {include_csv_path}")

    included = set()

    # Prefer original channel indices exported by this training script.
    if "original_channel_idx" in rows[0]:
        for row in rows:
            raw = str(row.get("original_channel_idx", "")).strip()
            if not raw:
                continue
            try:
                included.add(int(raw))
            except ValueError:
                continue
        if included:
            return included

    # Fallback: match predictor names from channel_map.
    if "predictor_name" not in rows[0]:
        raise RuntimeError(
            "Predictor include CSV must contain either 'original_channel_idx' or 'predictor_name' column."
        )

    for row in rows:
        key = str(row.get("predictor_name", "")).strip().lower()
        if not key:
            continue
        idxs = channel_map_by_predictor.get(key, [])
        included.update(idxs)

    if not included:
        raise RuntimeError(
            f"No channel indices resolved from predictor include CSV: {include_csv_path}"
        )
...
def compute_tile_class_counts(tile_ids, y):
    if len(tile_ids) != len(y):
        raise RuntimeError(
            f"tile_ids/y length mismatch: {len(tile_ids)} tile_ids vs {len(y)} y tiles"
        )

    tile_class_counts = {}
    for tid, arr in zip(tile_ids, y):
        counts = np.bincount(arr.reshape(-1), minlength=4).astype(np.int64)
        tile_class_counts[tid] = counts
    return tile_class_counts


def compute_block_class_counts(block_to_tiles, tile_class_counts):
    block_class_counts = {}
    for block, tids in block_to_tiles.items():
        counts = np.zeros(4, dtype=np.int64)
        for tid in tids:
            counts += tile_class_counts[tid]
        block_class_counts[block] = counts
    return block_class_counts


def _imbalance_score(projected_tile_loads, projected_class_loads, total_class_counts, class_weights):
    tile_score = float(np.std(projected_tile_loads))

    class_score = 0.0
    for class_id, weight in class_weights.items():
        total = float(total_class_counts[class_id])
        if total <= 0.0:
            continue
        class_share = projected_class_loads[:, class_id] / total
        class_score += float(weight) * float(np.std(class_share))

    return tile_score + class_score
...
def assign_blocks_to_folds(
    ...
    random_seed=42,
    mode="size_only",
    tile_class_counts=None,
    class_weights=None,
):
    """
    Greedy balancing of spatial blocks.
    - size_only: balance by number of tiles only
    - class_aware: balance by tiles plus class prevalence across folds
    """
    if mode not in {"size_only", "class_aware"}:
        raise ValueError("FOLD_ASSIGNMENT_MODE must be 'size_only' or 'class_aware'")
    ...
    if class_weights is None:
        class_weights = {1: 1.0, 2: 1.0, 3: 1.0}

    if mode == "class_aware":
        if tile_class_counts is None:
            raise RuntimeError("class_aware fold assignment requires tile_class_counts")
        block_class_counts = compute_block_class_counts(block_to_tiles, tile_class_counts)
        total_class_counts = np.zeros(4, dtype=np.int64)
        for counts in block_class_counts.values():
            total_class_counts += counts
        fold_class_loads = np.zeros((n_folds, 4), dtype=np.int64)
    else:
        block_class_counts = None
        total_class_counts = None
        fold_class_loads = None
    ...
    for b in blocks:
        if mode == "size_only":
            ...
            best_fold_id = None
            best_score = None
            for candidate_fold_id in range(n_folds):
                projected_tile_loads = np.array(fold_loads, dtype=np.float64)
                projected_tile_loads[candidate_fold_id] += block_sizes[b]

                projected_class_loads = fold_class_loads.copy()
                projected_class_loads[candidate_fold_id] += block_class_counts[b]

                score = _imbalance_score(
                    projected_tile_loads=projected_tile_loads,
                    projected_class_loads=projected_class_loads,
                ...
                if best_score is None or score < best_score:
                    best_score = score
                    best_fold_id = candidate_fold_id

            fold_id = int(best_fold_id)

        block_to_fold[b] = fold_id
        fold_loads[fold_id] += block_sizes[b]
        if mode == "class_aware":
            fold_class_loads[fold_id] += block_class_counts[b]

    print(f"[INFO] Fold tile counts (before buffering): {fold_loads}")
    if mode == "class_aware":
        for class_id in [1, 2, 3]:
            print(
                f"[INFO] Fold class-{class_id} pixel counts (before buffering): "
                f"{fold_class_loads[:, class_id].tolist()}"
            )
...
# --- Unified exclusion: names and indices ---
excluded_predictor_names, excluded_channel_idx_orig = parse_excluded_predictors_and_indices(CHANNELS_EXCL)
excluded_channel_idx_orig = set(excluded_channel_idx_orig)
included_channel_idx_orig = set()

need_channel_map = USE_CATEGORICAL_EMBEDDINGS or bool(excluded_predictor_names) or USE_PREDICTOR_INCLUDE_CSV
if need_channel_map and os.path.exists(CHANNEL_MAP_PATH):
...
if USE_PREDICTOR_INCLUDE_CSV:
    if channel_map_rows is None or channel_map_by_predictor is None:
        raise RuntimeError(
            "USE_PREDICTOR_INCLUDE_CSV requires channel_map.csv to be available."
        )
    included_channel_idx_orig = load_included_channel_indices_from_csv(
        PREDICTOR_INCLUDE_CSV,
        channel_map_by_predictor,
    )
    print(
        f"[INFO] Predictor include CSV selected {len(included_channel_idx_orig)} original channels "
        f"from {PREDICTOR_INCLUDE_CSV}"
    )

if USE_CATEGORICAL_EMBEDDINGS and channel_map_by_predictor is not None:

    quaternary_channel_idx_orig, quaternary_predictor_name_used = resolve_first_available_channel_idx(
        channel_map_by_predictor, QUATERNARY_PREDICTOR_NAMES, "quaternary"
    )
    landuse_channel_idx_orig, landuse_predictor_name_used = resolve_first_available_channel_idx(
        channel_map_by_predictor, LANDUSE_PREDICTOR_NAMES, "landuse"
    )

    print(
        f"Channel map selection -> {quaternary_predictor_name_used}: idx {quaternary_channel_idx_orig}, "
        f"{landuse_predictor_name_used}: idx {landuse_channel_idx_orig}"
    )
    embeddings_from_channel_map = True

if USE_PREDICTOR_INCLUDE_CSV and USE_CATEGORICAL_EMBEDDINGS:
    for required_idx in [quaternary_channel_idx_orig, landuse_channel_idx_orig]:
        if required_idx >= 0:
            included_channel_idx_orig.add(required_idx)
...
if excluded_predictor_names:
    ...
    for predictor_name in excluded_predictor_names:
        idxs = []
        matched_predictors = []
        for mapped_name, mapped_idxs in channel_map_by_predictor.items():
            if predictor_name in mapped_name:
                idxs.extend(mapped_idxs)
                matched_predictors.append(mapped_name)
        if not idxs:
            unresolved.append(predictor_name)
            continue
        unique_idxs = sorted(set(idxs))
        excluded_channel_idx_orig.update(unique_idxs)
        resolved_by_name[predictor_name] = {
            "predictors": sorted(set(matched_predictors)),
            "indices": unique_idxs,
        }
    if unresolved:
        raise RuntimeError(
    ...
    for token, info in resolved_by_name.items():
        print(
            f"[INFO] CHANNELS_EXCL token '{token}' matched {len(info['predictors'])} predictor(s) and {len(info['indices'])} channel(s)."
        )
...
if excluded_channel_idx_orig:
    print(f"[INFO] CHANNELS_EXCL (indices) will drop: {sorted(excluded_channel_idx_orig)}")

if included_channel_idx_orig:
    full_idx = set(range(len(channel_map_rows or [])))
    included_valid = {i for i in included_channel_idx_orig if i in full_idx}
    if not included_valid:
        raise RuntimeError("Predictor include CSV resolved zero valid channel indices for current X tensor.")
    excluded_channel_idx_orig.update(full_idx - included_valid)
    print(
        f"[INFO] Predictor include mode active: keeping {len(included_valid)} channels, "
        f"excluding {len(full_idx - included_valid)} channels"
    )
...
if USE_CATEGORICAL_EMBEDDINGS:
    print(f"Categorical embedding config active; class counts will be resolved from data.")
...
if X.ndim == 4:
    # Prefer unambiguous geometry-based detection first:
    # channels_first => (N, C, H, W) with H == W
    # channels_last  => (N, H, W, C) with H == W
    if X.shape[2] == X.shape[3] and X.shape[1] != X.shape[2]:
    ...
    elif X.shape[1] == X.shape[2] and X.shape[3] != X.shape[1]:
        print(f"[INFO] Data already in channels_last format: {X.shape}")
    elif channel_map_rows is not None and len(channel_map_rows) > 0:
        expected_channels = len(channel_map_rows)
        if X.shape[1] == expected_channels and X.shape[-1] != expected_channels:
            print(
                f"[INFO] Transposing X from {X.shape} using channel-map heuristic "
            ...
            print(
                f"[INFO] Data already in channels_last format: {X.shape} "
                f"(matches channel-map channels={expected_channels})"
            )
        else:
            raise RuntimeError(
                f"Unable to infer channel axis for X shape {X.shape} "
                f"with channel-map channels={expected_channels}."
            )
    else:
        raise RuntimeError(
            f"Unable to infer channel axis for X shape {X.shape}. "
            "Provide channel_map.csv or ensure data is in a standard layout."
        )
else:
    raise RuntimeError(f"Unexpected X ndim={X.ndim}; expected 4D tensor, got shape {X.shape}")

current_to_original_idx = list(range(X.shape[-1]))
...
if DROP_DERIVED_CHANNELS:
    ...
    X = X[..., keep_idx]
    current_to_original_idx = [current_to_original_idx[i] for i in keep_idx]
...
if USE_CATEGORICAL_EMBEDDINGS and embeddings_from_channel_map:
    ...
    ACTIVE_QUATERNARY_NUM_CLASSES = resolve_categorical_num_classes(
        quaternary_arr,
        QUATERNARY_NUM_CLASSES,
        "Quaternary",
    )
    ACTIVE_LANDUSE_NUM_CLASSES = resolve_categorical_num_classes(
        landuse_arr,
        LANDUSE_NUM_CLASSES,
        "Landuse",
    )
    ...
    quaternary_arr = np.where(
        (quaternary_arr < 0) | (quaternary_arr > ACTIVE_QUATERNARY_NUM_CLASSES),
        0,
        quaternary_arr,
    )
    landuse_arr = np.where(
        (landuse_arr < 0) | (landuse_arr > ACTIVE_LANDUSE_NUM_CLASSES),
        0,
        landuse_arr,
    )
...
elif USE_CATEGORICAL_EMBEDDINGS and not embeddings_from_channel_map:
    ACTIVE_QUATERNARY_NUM_CLASSES = resolve_categorical_num_classes(
        quaternary,
        QUATERNARY_NUM_CLASSES,
        "Quaternary",
    ...
    quaternary = np.where(
        (quaternary < 0) | (quaternary > ACTIVE_QUATERNARY_NUM_CLASSES),
        0,
        quaternary,
    ).astype(np.int32)
    landuse = np.where(
        (landuse < 0) | (landuse > ACTIVE_LANDUSE_NUM_CLASSES),
        0,
        landuse,
    ).astype(np.int32)
...
if USE_CATEGORICAL_EMBEDDINGS and embeddings_from_channel_map:
    embedding_rows = [
        {"branch": "quaternary_embedding", "orig_idx": quaternary_channel_idx_orig},
        {"branch": "landuse_embedding", "orig_idx": landuse_channel_idx_orig},
    ]

export_predictor_list_used(
    PREDICTOR_LIST_USED_CSV,
    channel_map_rows,
...
def build_unet_with_embeddings(input_shape, num_classes):
    ...
    # Use Reshape instead of Lambda(tf.squeeze) so the saved model can be reloaded
    # cleanly in newer Keras/TensorFlow environments without Lambda deserialization
    # or output-shape inference problems.
    qua_idx = Reshape(input_shape[:2], name="quaternary_squeeze")(qua_in)
    land_idx = Reshape(input_shape[:2], name="landuse_squeeze")(land_in)
...
class F1MetricsCallback(Callback):
    def __init__(self, Xv, yv, out_txt, prefix="", Xv_qua=None, Xv_land=None):
        super().__init__()
        self.Xv = Xv
        self.Xv_qua = Xv_qua
        ...
        self.prefix = prefix
        self.history = []
        self.pr_curve_dir = os.path.splitext(out_txt)[0] + "_diagnostics"
        os.makedirs(self.pr_curve_dir, exist_ok=True)

    def _build_pred_input(self):
    ...
    def _save_class3_pr_plot(self, precision, recall, ap, epoch):
        plt.figure(figsize=(6, 6))
        plt.plot(recall, precision, label=f"AP={ap:.4f}")
        plt.xlabel("Recall")
        plt.ylabel("Precision")
        plt.title(f"Class 3 PR Curve ({self.prefix}) - Epoch {epoch+1}")
        plt.legend()
        plt.tight_layout()
        out_png = os.path.join(self.pr_curve_dir, f"class3_pr_epoch_{epoch+1:03d}.png")
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        pred_input = self._build_pred_input()
        y_prob = self.model.predict(pred_input, batch_size=2, verbose=0)
        ...
        true_counts = {cls: int(np.sum(y_true == cls)) for cls in [1, 2, 3]}
        pred_counts = {cls: int(np.sum(y_pred_valid == cls)) for cls in [1, 2, 3]}

        for cls in [1, 2, 3]:
            tp = np.sum((y_true == cls) & (y_pred_valid == cls))
            fp = np.sum((y_true != cls) & (y_pred_valid == cls))
            fn = np.sum((y_true == cls) & (y_pred_valid != cls))
            tn = np.sum((y_true != cls) & (y_pred_valid != cls))
            ...
            f1s.append(f1)
            recalls.append(rec)
            ep_dict[f"precision_{cls}"] = prec
            ep_dict[f"recall_{cls}"] = rec
            ep_dict[f"specificity_{cls}"] = spec
            ep_dict[f"f1_{cls}"] = f1
            ep_dict[f"support_true_{cls}"] = true_counts[cls]
            ep_dict[f"support_pred_{cls}"] = pred_counts[cls]

        ep_dict["macro_f1"] = float(np.mean(f1s))
        ...
        acc = np.sum(y_pred_valid == y_true) / (len(y_true) + eps)
        ...
        # Class-3 focused diagnostics
        y_true_class3 = (y_true == 3).astype(np.uint8)
        class3_prob = y_prob_valid[..., 2].astype(np.float32)
        ap3 = float(average_precision_score(y_true_class3, class3_prob)) if np.any(y_true_class3) else np.nan
        precision3_curve, recall3_curve, _ = precision_recall_curve(y_true_class3, class3_prob)
        self._save_class3_pr_plot(precision3_curve, recall3_curve, ap3, epoch)

        inside = class3_prob[y_true == 3]
        outside = class3_prob[y_true != 3]
        ep_dict["class3_ap"] = ap3
        ep_dict["class3_prob_mean_inside"] = float(np.mean(inside)) if inside.size else np.nan
        ep_dict["class3_prob_median_inside"] = float(np.median(inside)) if inside.size else np.nan
        ep_dict["class3_prob_mean_outside"] = float(np.mean(outside)) if outside.size else np.nan
        ep_dict["class3_prob_median_outside"] = float(np.median(outside)) if outside.size else np.nan
        ep_dict["class3_prob_q90_inside"] = float(np.quantile(inside, 0.90)) if inside.size else np.nan
        ep_dict["class3_prob_q90_outside"] = float(np.quantile(outside, 0.90)) if outside.size else np.nan

        print(
            f"  Class-3 diagnostics: precision={ep_dict['precision_3']:.4f}, recall={ep_dict['recall_3']:.4f}, "
            f"AP={ep_dict['class3_ap']:.4f}, support_true={ep_dict['support_true_3']}, support_pred={ep_dict['support_pred_3']}",
            flush=True,
        )
        print(
            f"  Class-3 prob inside vs outside: mean_in={ep_dict['class3_prob_mean_inside']:.4f}, "
            f"mean_out={ep_dict['class3_prob_mean_outside']:.4f}, q90_in={ep_dict['class3_prob_q90_inside']:.4f}, "
            f"q90_out={ep_dict['class3_prob_q90_outside']:.4f}",
            flush=True,
        )
...
def create_dataset(X, y, batch_size=2, shuffle=True, X_qua=None, X_land=None):
    # Stream batches from numpy arrays instead of creating one giant Tensor.
    # This avoids large CPU->GPU constant copies that can fail after fold 1.
    X = X.astype(np.float32, copy=False)
    y = y.astype(np.float32, copy=False)

    has_embeddings = X_qua is not None and X_land is not None
    if has_embeddings:
        X_qua = X_qua.astype(np.int32, copy=False)
        X_land = X_land.astype(np.int32, copy=False)

    n_samples = len(X)
    base_indices = np.arange(n_samples, dtype=np.int32)
        ...
        for start in range(0, n_samples, batch_size):
            ...
            if has_embeddings:
                yield (
                    {
                        "cont_in": X[bidx],
                        "quaternary_in": X_qua[bidx],
                        "landuse_in": X_land[bidx],
                    },
                    y[bidx],
                )
            else:
    ...
    if has_embeddings:
        output_signature = (
            {
                "cont_in": tf.TensorSpec(shape=(None,) + X.shape[1:], dtype=tf.float32),
                "quaternary_in": tf.TensorSpec(shape=(None,) + X_qua.shape[1:], dtype=tf.int32),
                "landuse_in": tf.TensorSpec(shape=(None,) + X_land.shape[1:], dtype=tf.int32),
            },
    ...
    dataset = tf.data.Dataset.from_generator(_gen, output_signature=output_signature)
...
if DO_SPATIAL_CV:
    ...
    print(f"Fold assignment mode: {FOLD_ASSIGNMENT_MODE}")
    ...
    for fold in range(N_FOLDS):
        ...
        print(f"Saving best CV model to native Keras checkpoint: {model_path}")
...
if DO_MAIN_TRAIN:
    ...
    print(f"Saving best main model to native Keras checkpoint: {ckpt_path}")
    ckpt = ModelCheckpoint(
        ckpt_path,
        monitor="val_macro_f1",
        mode="max",
    ...
    main_early_stop = EarlyStopping(
        monitor="val_macro_f1",
        mode="max",
        patience=MAIN_EARLY_STOP_PATIENCE,
        restore_best_weights=True,
        verbose=1,
    )

    main_lr_scheduler = ReduceLROnPlateau(
        monitor="val_macro_f1",
        mode="max",
        factor=MAIN_LR_FACTOR,
        patience=MAIN_LR_PATIENCE,
        min_lr=MAIN_MIN_LR,
        verbose=1,
    )
    ...
    history = model.fit(
        ...
        validation_data=val_dataset,
        epochs=40,
        callbacks=[f1cb, ckpt, main_early_stop, main_lr_scheduler, TerminateOnNaN()],
        verbose=1,
    )
