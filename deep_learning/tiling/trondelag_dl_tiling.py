"""
Deep Learning Tiling Script for Trondelag Test Area
Adapted from Tiles_Spyder_ed_2026.py
...
# ============================================================
# USER PATHS - TRONDELAG TEST
# ============================================================
PREDICTOR_DIR = r"E:\Test\Trondelag_test\Predictors"
...
OUT_DIR       = r"E:\Test\Trondelag_test\DL_tiles"

# Reference grid (snap raster with water as NoData)
DTM_MASK = r"E:\Test\Trondelag_test\Mask\Trondelag_mask_aligned.tif"

# Robust stats for Sentinel bands (computed by sentinel_robust_normalization.py)
SENTINEL_STATS_CSV = r"E:\Test\Trondelag_test\stats_norm\global_robust_stats_landonly_trimmed_weighted.csv"

# Directory containing masked Sentinel tiles (separate from PREDICTOR_DIR)
SENTINEL_MASKED_DIR = r"E:\Test\Trondelag_test\masked"
...
if os.path.exists(SENTINEL_MASKED_DIR):
    ...
    # Create temp VRT in OUT_DIR to avoid it being picked up by find_predictors()
    temp_vrt_name = os.path.basename(sentinel_vrt).replace('.vrt', '_absolute_paths.vrt')
    temp_vrt = os.path.join(OUT_DIR, temp_vrt_name)
    # Replace relative paths like "Trondelag_tile_XX_YY_masked.tif" with full paths
    updated_content = re.sub(
        r'(SourceFilename.*?)Trondelag_tile_\d{2}_\d{2}_masked\.tif',
    ...
    updated_content = re.sub(
        r'SourceFilename relativeToVRT="[01]">([^<]+Trondelag_tile[^<]*\.tif)',
