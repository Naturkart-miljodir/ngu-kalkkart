...
MODEL_PATH = Path(
    r"/home/acosta_pedro/Outputs/Models/XGBoost/Categorical/Base_Not_Tunned/xgb_final_model.joblib"
)
...
OUTPUT_DIR = Path(
    r"/home/acosta_pedro/Outputs/Models/XGBoost/Categorical/Base_Not_Tunned/Production"
)

CHANNEL_MAP_PATH = Path(
    r"/home/acosta_pedro/Classical_ML/Predictors_file/predictors_used_for_modelling.csv"
)
...
def predict_proba_xgb_or_orf(model, X_valid: np.ndarray):
    ...
    if hasattr(model, "predict_proba") and hasattr(model, "classes_"):
        # Native XGBoost categorical inference can fail when unseen categories appear at predict time.
    ...
    raise RuntimeError("Unsupported model type: expected XGBoost with predict_proba or ORF bundle dict")


def configure_inference_backend(model):
    """Align inference backend with CPU-resident inputs to avoid XGBoost device mismatch fallback."""
    try:
    ...
    if isinstance(model, dict) and model.get("model_type") in {"xgb_orf_style", "orf_style"}:
        for rec in model.get("models", []):
            sub_model = rec.get("model")
            if hasattr(sub_model, "set_params"):
                try:
                    sub_model.set_params(device="cpu")
                except Exception:
                    pass
    return model
...
def main():
    ...
    try:
        ...
        uncertainty_paths = {
            method: OUTPUT_DIR / f"xgb_uncertainty_{method}.tif"
        ...
        uncertainty_tmp_paths = {
            method: OUTPUT_DIR / f"_tmp_xgb_uncertainty_{method}.tif"
            for method in uncertainty_methods
        }
            ...
            for row_off in row_starts:
                for col_off in col_starts:
                    ...
                    if n_valid <= PREDICT_CHUNK_SIZE:
                        ...
                        probs, pred_classes = predict_proba_xgb_or_orf(model, X_valid_model)
                        ...
                        for start in range(0, n_valid, PREDICT_CHUNK_SIZE):
                            ...
                            probs_chunk, pred_classes = predict_proba_xgb_or_orf(model, X_chunk_model)
                            if not np.array_equal(pred_classes, class_labels):
                    ...
                    pred_class_flat[valid_mask] = class_labels[np.argmax(probs, axis=1)].astype(np.float32, copy=False)
