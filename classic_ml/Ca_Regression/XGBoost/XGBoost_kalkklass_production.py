...
# -*- coding: utf-8 -*-
"""
Apply trained XGBoost Klass_Ca classifier to raster covariates and produce:
...
Requirements:
- Model: joblib bundle from XGBoost_window_Kalkklass_modelling.py with keys
...
def build_channel_map_from_importance(importance_path: Path, predictor_dir: Path, output_path: Path) -> Path:
    """Build a direct raster map from the model's Variable column."""
...
# Here we use the 250K model from the Geo_topo_geofys run.
MODEL_VARIANT = "Only_Geovariabler/250k/Balanced_Non_ordinal"

KLASS_CA_MODEL_ROOT = Path(r"C:\Users\acosta_pedro\OneDrive - Norges geologiske undersøkelse\Geochemistry NGU_2026\Kalk_project\Modelling\XGBoost\KlassCa_modelling")
MODEL_PATH = str(KLASS_CA_MODEL_ROOT / MODEL_VARIANT / "xgb_cls_final_model_Kalkklass.joblib")

COVARIATE_DIR = r"G:\Covariates_to_model"

PRODUCTION_OUT_DIR = Path(r"G:\National_maps\Ca_class\XGBoost\250K\V2")
...
try:
    ...
    IMPORTANCE_PATH = Path(
        ...
        r"Geochemistry NGU_2026/Kalk_project/Modelling/XGBoost/KlassCa_modelling/"
        r"Only_Geovariabler/250k/Balanced_Non_ordinal/variable_importance.csv"
    )
...
if model_type not in {"unknown", "xgboost", "xgboost_classifier"}:
    print(f"[WARNING] model_type='{model_type}' in bundle (expected 'xgboost_classifier').")
else:
...
def predict_model_proba(X):
    """Return class probabilities for ordinary or ordinal XGBoost bundles."""
