...
"""
Random Forest multi-class classification for Kalkklass from a prebuilt regression matrix.
...
Outputs per run:
  ...
  - final_model_train_metrics.txt
    - rf_cls_final_model_Kalkklass.joblib
...
OUT_DIR = r"C:\Users\acosta_pedro\OneDrive - Norges geologiske undersøkelse\Geochemistry NGU_2026\Kalk_project\Modelling\RandomForest\KlassCa_modelling\Only_Geovariabler\Unbalanced_SHAP_NonOrdinal"
...
# === Random Forest hyperparameters (baseline; used when ENABLE_RF_TUNING=False) ===
N_ESTIMATORS = 500
MAX_DEPTH = 5
MIN_SAMPLES_LEAF = 2
MAX_FEATURES = "sqrt"
...
# =============================================================================
# HELPERS â€” Random Forest fit / tuning
# =============================================================================
...
TUNED_RF_PARAMS: Dict[str, object] = {}
...
def save_shap_outputs(model, X, var_names, out_dir):
    ...
    models = model if USE_ORDINAL_RF else [model]
    value_frames = []
    summary_frames = []
    for model_idx, fitted_model in enumerate(models, start=1):
        ...
        if values.ndim == 3:
            values = values[:, :, -1]
        prefix = f"threshold_{model_idx}_" if USE_ORDINAL_RF else ""
...
def run_cv_and_save(X_samp, y_samp_1based, x_samp, y_coord_samp,
    ...
    active_params = dict(DEFAULT_RF_PARAMS)
    if xgb_params:
    ...
    with open(os.path.join(out_dir, "regression_report.txt"), "w", encoding="utf-8") as f:
        f.write("Random Forest classification â€” target: Kalkklass_int\n")
        f.write("=" * 60 + "\n")
    ...
    try:
        import joblib
        model_path = os.path.join(out_dir, "rf_cls_final_model_Kalkklass.joblib")
        joblib.dump({
            "model": final_model,
            "model_type": "random_forest_classifier",
            "ordinal": USE_ORDINAL_RF,
