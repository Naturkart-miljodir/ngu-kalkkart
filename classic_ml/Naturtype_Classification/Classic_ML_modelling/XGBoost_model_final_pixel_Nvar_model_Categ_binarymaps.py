...
OUT_DIR = r"D:\Classical_ML_models\Test\XGBoost\Categorical_maps\Ca_prob\Non_ORF_style\clean\Tunned"
...
# Categorical handling for XGBoost-compatible treatment
...
def _new_xgb_classifier(use_gpu: bool, random_state: int, objective: str, num_class: int = None, xgb_params: Dict[str, Any] = None):
...
def _fit_predict_orf_style_xgb(
    ...
    return pred, out_model, "xgboost_orf"
...
def fit_predict_xgb(
    ...
    try:
        ...
        if sample_weight is not None:
            model.fit(X_tr, y_tr_idx, sample_weight=sample_weight)
        else:
            model.fit(X_tr, y_tr_idx)
        pred_idx = model.predict(X_te).astype(np.int32)
        pred = np.asarray([idx_to_class[int(v)] for v in pred_idx], dtype=np.int32)
        backend = "xgboost_gpu" if use_gpu else "xgboost_cpu"
        return pred, model, backend
...
for fold, (train_idx, test_idx) in enumerate(gkf.split(X_sample, y_sample, groups), start=1):
    ...
    y_pred, model, backend = fit_predict_xgb(
        X_tr,
        y_tr,
        X_te,
        sample_weight=sample_weight,
...
_, final_model, final_backend = fit_predict_xgb(
    X_final,
    y_sample,
...
if RUN_TOP_N_SUBSET_MODELS:
    ...
    print("[INFO] RUN_TOP_N_SUBSET_MODELS=False: skipping Top 10/20/30 subset model runs.")
