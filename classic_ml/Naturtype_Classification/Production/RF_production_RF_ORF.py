...
MODEL_PATH = Path(
    r"D:\Classical_ML_models\RF\Class_Ca_prob\All\rf_final_model.joblib"
)
...
OUTPUT_DIR = Path(
    r"D:\Classical_ML_models\RF\Class_Ca_prob\All\Production"
)
...
def predict_proba_rf_or_orf(model, X_valid: np.ndarray):
    """Return class probabilities for either a plain RF model or ORF-style model bundle."""
    if hasattr(model, "predict_proba") and hasattr(model, "classes_"):
        probs = model.predict_proba(X_valid).astype(np.float32)
        classes = np.asarray(model.classes_, dtype=np.int32)
        return probs, classes

    if isinstance(model, dict) and model.get("model_type") == "orf_style":
...
def main():
    ...
    try:
        ...
        # Probe first sample to infer class ordering for RF and ORF paths alike.
        ...
        try:
            ...
            for row_off in row_starts:
                for col_off in col_starts:
                    ...
                    probs, _ = predict_proba_rf_or_orf(model, X_valid)
